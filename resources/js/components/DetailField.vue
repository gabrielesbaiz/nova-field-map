<template>
  <div>
    ...
    <div :id="uniqueMapId" :style="'height:' + mapHeight"></div>
  </div>
</template>
...
export default {
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            ...
            const geoLayer = L.geoJson(this.geoJsons);
  ...
  data() {
    ...
    // Unique Map ID
    const uniqueMapId = `map-${Math.random().toString(36).substring(2, 15)}`;
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      // OpenStreetMap
      if (!this.tileProviders["OpenStreetMap"]) {
      ...
      // Layer control
      L.control
        .layers(this.tileProviders, null, {
          position: "topright",
        })
        .addTo(this.map);
      ...
      // Add layer controls
      L.control
        .layers(this.tileProviders, null, { position: "topright" })
        .addTo(this.map);

      // Trigger resize when the map's container becomes visible
